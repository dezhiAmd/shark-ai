...
class LlmTaskInput:
    ...
    def seq_stride(self) -> int:
        return self.model_params.paged_kv_cache.block_seq_stride

    @property
    def batch_seq_len(self):
        seq_stride = self.seq_stride
        if self.model_params.chunk_prefill_size is not None:
            assert self.model_params.chunk_prefill_size == int(math.ceil(self.model_params.chunk_prefill_size / seq_stride) * seq_stride)
            return self._model_params.chunk_prefill_size
        else:
